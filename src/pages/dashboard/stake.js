    ...
    <div className="bg-gray-100 overflow-hidden">
      ...
      <div className="mx-auto w-full max-w-2xl gap-x-4 gap-y-1 grid grid-cols-2">
        ...
        <CryptoPaymentModal
          ...
          address={bitcoin}
          clipboardHandler={clipboardBTC}
          handleChange={handleChangeBTC}
          depositAmount={deposit_btc}
          depositHandler={() => depositBTC({supabase, deposit_btc, user, setLoading, setOpenBTC})}
          loading={loading}
        />
        {/* XRP */}
        <CryptoPaymentModal
          isOpen={openXRP}
          onClose={closeXRPModal}
          cryptocurrency="XRP"
          cryptoIcon={<FaBitcoin className="mt-1" />}
          inputValue={inputValue}
          address={xrp}
          clipboardHandler={clipboardXRP}
          handleChange={handleChangeXRP}
          depositAmount={deposit_xrp}
          depositHandler={() => depositXRP({ supabase, user, setLoading, setOpenXRP, deposit_xrp })}
          loading={loading}
        />
        {/* ETH */}
        <CryptoPaymentModal
...
{/* supabase, user, setLoading, setOpenBTC */}
        ...
        {/* hot */}
        <Transition appear show={openHot} as={Fragment}>
        <Dialog as="div" className="relative z-10" onClose={closeHotModal}>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
            ...
            <Transition.Child
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-center">Complete transaction by sending the exact amount of <span className="font-bold">{inputValue}{" "}$hot</span> to the address below</p>
                    {/* <img 
                      className="w-1/3 m-auto"
                      src="https://bpvsklhytoplnehaskcs.supabase.co/storage/v1/object/sign/avatars/usdt.png?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJhdmF0YXJzL3VzZHQucG5nIiwiaWF0IjoxNjg2NTc2NzQ2LCJleHAiOjE3MTgxMTI3NDZ9.Wb2-YuO351IVf8XQGa-FCz7lrMWrSanD-g6ZESJCm94&t=2023-06-12T13%3A32%3A25.710Z"
                    /> */}
                    <p className="text-center uppercase text-sm text-gray-400 my-3">$hot Address</p>
                    <div className="text-sm font-medium text-gray-500 border rounded-lg flex justify-between p-2 px-3">
                      <span className="flex space-x-2">
                        <FaBitcoin className='mt-1' />
                        <p>{hot}</p>
                  ...
                  <div className="space-y-2">
                    <button
                        type="submit"
                        onClick={() => depositHot({ supabase, user, setLoading, setOpenHot, deposit_hot })}
                        className="inline-flex w-full justify-center rounded-md border border-transparent bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                        disabled={loading}
                      >
                      {loading ? 'Loading ...' : 'Pay $hot'}
                    </button>
                    <button
                      type="button"
                      className="inline-flex w-full justify-center rounded-md border-2 border-blue-500 px-4 py-2 text-sm font-medium text-blue-500 hover:bg-blue-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                      onClick={closeHotModal}
                    >
                      Pay Later
                    </button>
                  </div>
                  ...
                  <div className="mt-3">
                    <div className="text-rose-500 flex space-x-2">
                      <AiOutlineInfoCircle />
                      <p className='text-xs'>Be aware that this order will be cancelled if you send any other $hot amount.</p>
                    </div>
                    <div className="flex space-x-2 text-gray-500">
                      <AiOutlineInfoCircle />
                      <p className='text-xs'>Account will be credited once we received your payment.</p>
